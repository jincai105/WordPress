 ...
 */
function mercia_theme_addons_setup() {

	// Add theme support for Mercia Pro plugin.
	add_theme_support( 'mercia-pro' );
	...
	add_theme_support( 'infinite-scroll', array(
		...
		'render'         => 'mercia_infinite_scroll_render',
	) );
 ...
 */
function mercia_theme_addons_scripts() {
	...
	if ( is_active_widget( 'TZWB_Facebook_Likebox_Widget', false, 'tzwb-facebook-likebox' )
		...
		wp_enqueue_style( 'mercia-themezee-widget-bundle', get_template_directory_uri() . '/assets/css/themezee-widget-bundle.css', array(), '20160421' );
	...
	if ( is_singular( 'post' ) ) {
		...
		wp_enqueue_style( 'mercia-themezee-related-posts', get_template_directory_uri() . '/assets/css/themezee-related-posts.css', array(), '20160421' );

	}

}
add_action( 'wp_enqueue_scripts', 'mercia_theme_addons_scripts' );
 ...
 */
function mercia_wrapper_start() {
	echo '<section id="primary" class="content-area">';
	echo '<main id="main" class="site-main" role="main">';
}
remove_action( 'woocommerce_before_main_content', 'woocommerce_output_content_wrapper', 10 );
add_action( 'woocommerce_before_main_content', 'mercia_wrapper_start', 10 );


/**
 * Set wrapper end for wooCommerce
 */
function mercia_wrapper_end() {
	echo '</main><!-- #main -->';
