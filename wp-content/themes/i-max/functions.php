...
function imax_scripts_styles() {
	...
	wp_enqueue_style( 'imax-style', get_stylesheet_uri(), array(), '2016-07-22' );
...
function imax_body_class( $classes ) {
	...
	if( ! empty($imax_page_class) )
		$classes[] = esc_attr($imax_page_class);
	
	if( rwmb_meta('imax_page_nopad') == 1 )
		$classes[] = 'tx-nopad';
...
function imax_customize_register( $wp_customize ) {
	...
	$wp_customize->selective_refresh->add_partial( 'slidersettings', array(
		'selector' => '.da-slider .nx-slider .nx-slider-container h2',
	...
	$wp_customize->selective_refresh->add_partial( 'search-icon', array(
		'selector' => '.topsearch',
...
function imax_customize_preview_js() {
	wp_enqueue_script( 'imax-customizer', get_template_directory_uri() . '/js/theme-customizer.js', array( 'customize-preview' ), '1.0.2', true );
}
add_action( 'customize_preview_init', 'imax_customize_preview_js' );

function imax_customizer_control() {
...
add_action( 'customize_controls_enqueue_scripts', 'imax_customizer_control' );
...
function imax_import_files() {
  return array(
  	/**/
    array(
      'import_file_name'             	=> 'i-MAX Demo Classic',
      'import_file_url'            		=> 'https://raw.githubusercontent.com/TemplatesNext/i-max-demo/master/i-max.wordpress-1.xml',
      'import_widget_file_url'     		=> 'https://raw.githubusercontent.com/TemplatesNext/i-max-demo/master/i-max-widgets.wie',
      'import_customizer_file_url' 		=> 'https://raw.githubusercontent.com/TemplatesNext/i-max-demo/master/i-max-export-1.dat',
      'import_preview_image_url'     	=> 'https://raw.githubusercontent.com/TemplatesNext/i-max-demo/master/i-max-demo-1.jpg',
      'import_notice'                	=> __( 'Please make sure you have plugin "TemplatesNext ToolKit" and "Contact Form 7" installed and active before you start the import process. <br> This process involves transfer of data and media from server to server and might take some time.', 'i-max' ),
	  'preview_url'                		=> 'http://templatesnext.org/imax/',
    ),
	
    array(
      'import_file_name'             	=> 'i-MAX Demo - Page Builder',
      'import_file_url'            		=> 'https://raw.githubusercontent.com/TemplatesNext/i-max-demo/master/i-max.wordpress-2.xml',
      'import_widget_file_url'     		=> 'https://raw.githubusercontent.com/TemplatesNext/i-max-demo/master/i-max-widgets.wie',
      'import_customizer_file_url' 		=> 'https://raw.githubusercontent.com/TemplatesNext/i-max-demo/master/i-max-export-2.dat',
      'import_preview_image_url'     	=> 'https://raw.githubusercontent.com/TemplatesNext/i-max-demo/master/i-max-demo-2.jpg',
      'import_notice'                	=> __( 'Please make sure you have plugin "TemplatesNext ToolKit" and "Contact Form 7" installed and active before you start the import process. <br> This process involves transfer of data and media from server to server and might take some time.', 'i-max' ),
	  'preview_url'                		=> 'http://templatesnext.org/imax/?page_id=688',	  
    ),	
  );
}
add_filter( 'pt-ocdi/import_files', 'imax_import_files' );

add_filter( 'pt-ocdi/disable_pt_branding', '__return_true' );

function imax_after_import_setup($selected_import) {
		if ( 'i-MAX Demo Classic' === $selected_import['import_file_name'] ) {
	...
	} elseif ( 'i-MAX Demo - Page Builder' === $selected_import['import_file_name'] ) {
...
add_action( 'pt-ocdi/after_import', 'imax_after_import_setup' );
...
function ocdi_plugin_page_setup( $default_settings ) {
    $default_settings['page_title']  = esc_html__( 'i-MAX One Click Demo Set-up', 'i-max' );
    $default_settings['menu_title']  = esc_html__( 'i-MAX Demo Setup' ,'i-max' );
...
* Admin notice
*/
add_action('admin_notices', 'imax_admin_notice_005');
function imax_admin_notice_005() {
    ...
    if ( ! get_user_meta($user_id, 'imax_ignore_notice_005') ) {
        echo '<div class="updated imax-notice" style="display: none;"><p><div style="line-height: 20px;">'; 
				...
				printf(__('<br><a href="%1$s" target="_blank" class="ad-review">Post Your Review</a> <a href="%2$s" class="tx-dismiss">Remind Later</a><div class="clear"></div>', 'i-max' ), $notice_url, '?imax_notice_ignore_005=0');								
        echo "</div></p></div>";
    }
}

add_action('admin_init', 'imax_notice_ignore_005');
function imax_notice_ignore_005() {
    global $current_user;
	$user_id = $current_user->ID;
    /* If user clicks to ignore the notice, add that to their user meta */
	if ( isset($_GET['imax_notice_ignore_005']) && '0' == $_GET['imax_notice_ignore_005'] ) {
    	add_user_meta($user_id, 'imax_ignore_notice_005', 'true', true);
    }
}
